...
async fn request(verb: &str, path: &str, body: Option<&str>, tok: Option<&str>) -> bool {
    async fn with_error(verb: &str, path: &str, body: Option<&str>, tok: Option<&str>) -> Result<JsValue, JsValue> {
    ...
    let res = with_error(verb, path, body, tok).await;
    log(&format!("{:?}", res));
    res.is_ok()
}
