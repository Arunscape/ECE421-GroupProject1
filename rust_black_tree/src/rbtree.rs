    ...
    T: std::fmt::Debug,
{
    ...
    pub fn contains(&self, val: &T) -> bool {
        let n = self.find(val);
        &ColourNode::get(&self.data, n).value == val
    }

    pub fn insert(&mut self, val: T) {
        if let Some(_root) = self.root {
            let n = self.find(&val);
            let node = ColourNode::get(&self.data, n);
            if node.value == val {
                ...
                ColourNode::set_child(&mut self.data, n, node, side);
    ...
    pub fn fix_del_color(&mut self, n: usize, child: usize) {
        dbg!("Fix_del_color");
        if !ColourNode::get(&self.data, n).is_red() {
            if ColourNode::get(&self.data, child).is_red() {
                ColourNode::get_mut(&mut self.data, child).color = Color::Black;
            } else {
    ...
    fn set_maybe_black(&mut self, no: Option<usize>) {
        if let Some(n) = no {
            ColourNode::get_mut(&mut self.data, n).color = Color::Black;
        }
    }

    fn delete_case_1(&mut self, n: usize) {
        dbg!("delete case 1");
        if ColourNode::get(&self.data, n).parent.is_some() {
    ...
    fn delete_case_2(&mut self, n: usize) {
        dbg!("delete case 2");
        let s = ColourNode::get(&self.data, n).get_sibling(&self.data);
        if ColourNode::get(&self.data, n).is_sibling_black(&self.data) {
            let p = ColourNode::get(&self.data, n).parent.expect("D2 P");
            self.set_maybe_black(s);
            ColourNode::get_mut(&mut self.data, p).color = Color::Red;
            self.rotate(ColourNode::get(&self.data, n).side(&self.data), p);
        }
    ...
    fn delete_case_3(&mut self, n: usize) {
        dbg!("delete case 3");
        let s = ColourNode::get(&self.data, n)
            .get_sibling(&self.data)
            .expect("D3 S");
        let p = ColourNode::get(&self.data, n).parent.expect("D3 P");
        if ColourNode::get(&self.data, n).is_parent_black(&self.data)
            && !ColourNode::get(&self.data, s).is_red()
            && ColourNode::get(&self.data, s).is_child_black(&self.data, Side::Left)
            && ColourNode::get(&self.data, s).is_child_black(&self.data, Side::Right)
        {
    ...
    fn delete_case_4(&mut self, n: usize) {
        ...
        if !node.is_parent_black(&self.data)
            ...
            && ColourNode::get(&self.data, s).is_child_black(&self.data, Side::Right)
        {
            ColourNode::get_mut(&mut self.data, s).color = Color::Red;
            ColourNode::get_mut(&mut self.data, p).color = Color::Black;
        } else {
    ...
    fn delete_case_5(&mut self, n: usize) {
        ...
        let s = ColourNode::get(&self.data, n)
            .get_sibling(&self.data)
            .expect("D5 S");
        if !ColourNode::get(&self.data, s).is_red() {
            if ColourNode::get(&self.data, n).is_child(&self.data, Side::Left)
                && ColourNode::get(&self.data, s).is_child_black(&self.data, Side::Right)
                && !ColourNode::get(&self.data, s).is_child_black(&self.data, Side::Left)
            {
                let scl = ColourNode::get(&self.data, s).get_child(Side::Left);
                ColourNode::get_mut(&mut self.data, s).color = Color::Red;
                self.set_maybe_black(scl);
                self.rotate(Side::Right, s);
            } else if ColourNode::get(&self.data, n).is_child(&self.data, Side::Right)
                && ColourNode::get(&self.data, s).is_child_black(&self.data, Side::Left)
                && !ColourNode::get(&self.data, s).is_child_black(&self.data, Side::Right)
            {
                let scr = ColourNode::get(&self.data, s).get_child(Side::Right);
                ColourNode::get_mut(&mut self.data, s).color = Color::Red;
    ...
    fn delete_case_6(&mut self, n: usize) {
        ...
        let s = ColourNode::get(&self.data, n)
            .get_sibling(&self.data)
            .expect("D6 S");
        let p = ColourNode::get(&self.data, n).parent.expect("D6 P");
        let pc = ColourNode::get(&self.data, p).color;
        ColourNode::get_mut(&mut self.data, s).color = pc;
        ColourNode::get_mut(&mut self.data, p).color = Color::Black;
    ...
    fn delete_replace(&mut self, n: usize) -> usize {
        ...
        match (node.lchild, node.rchild) {
            (Some(lc), Some(rc)) => {
                let p = node.parent;
                let successor = ColourNode::get(&self.data, rc).find_min(&self.data);
                self.delete_replace(successor);
                self.data.swap(successor, n);

                ColourNode::get_mut(&mut self.data, n).lchild = Some(lc);
                ColourNode::get_mut(&mut self.data, n).rchild = Some(rc);
                ColourNode::get_mut(&mut self.data, n).parent = p;
                ColourNode::get_mut(&mut self.data, n).ptr = n;
                return successor;
            }
            (None, Some(_rc)) => self.replace_node(n, ColourNode::get(&self.data, n).rchild),
            (Some(_lc), None) => self.replace_node(n, ColourNode::get(&self.data, n).lchild),
    ...
    fn replace_node(&mut self, to_delete: usize, to_attach: Option<usize>) {
        let node = ColourNode::get(&self.data, to_delete);
        if let Some(p) = node.parent {
            if node.is_child(&self.data, Side::Left) {
                ColourNode::get_mut(&mut self.data, p).lchild = to_attach;
            } else {
                ColourNode::get_mut(&mut self.data, p).rchild = to_attach;
            }
        } else {
            self.root = to_attach;
        }
    ...
    pub fn to_string(&self) -> String {
        if let Some(root) = self.root {
            ColourNode::get(&self.data, root).to_string(&self.data)
        } else {
    ...
    pub fn to_pretty_string(&self) -> String {
        if let Some(root) = self.root {
            ColourNode::get(&self.data, root).to_pretty_string(&self.data, 0)
        } else {
            String::from("(Empty tree)")
        }
    }
    ...
    fn fix_ins_color(&mut self, n: usize) {
        ColourNode::get_mut(&mut self.data, n).color = Color::Red;
        if let Some(p) = ColourNode::get(&self.data, n).parent {
            if !ColourNode::get(&self.data, p).is_red() {
                // parent is black
                // do nothing
            } else if ColourNode::get(&self.data, n)
                .get_uncle(&self.data)
                .is_some()
                && ColourNode::get(
                    &self.data,
                    ColourNode::get(&self.data, n)
                        .get_uncle(&self.data)
                        .unwrap(),
                )
                .is_red()
            {
                ...
                let p = ColourNode::get(&self.data, n).parent.unwrap();
                let u = ColourNode::get(&self.data, n)
                    .get_uncle(&self.data)
                    .unwrap();
                ColourNode::get_mut(&mut self.data, p).color = Color::Black;
                ColourNode::get_mut(&mut self.data, u).color = Color::Black;
                self.fix_ins_color(ColourNode::get(&self.data, p).parent.unwrap());
            } else {
                // uncle is black
        ...
        ColourNode::get_mut(&mut self.data, self.root.unwrap()).color = Color::Black;
    }

    fn do_ins_hard_case(&mut self, nn: usize) {
        let mut n = nn;
        let mut p = ColourNode::get(&self.data, n).parent.unwrap();
        if ColourNode::get(&self.data, p).is_child(&self.data, Side::Left)
            && ColourNode::get(&self.data, n).is_child(&self.data, Side::Right)
        {
            self.rotate(Side::Left, n);
            n = ColourNode::get(&self.data, n)
                .get_child(Side::Left)
                .unwrap();
        }

        p = ColourNode::get(&self.data, n).parent.unwrap();
        if ColourNode::get(&self.data, p).is_child(&self.data, Side::Right)
            && ColourNode::get(&self.data, n).is_child(&self.data, Side::Left)
        {
    ...
    fn do_ins_hard_case2(&mut self, n: usize) {
        let p = ColourNode::get(&self.data, n).parent.unwrap();
        let g = ColourNode::get(&self.data, p).parent.unwrap();

        ColourNode::get_mut(&mut self.data, p).color = Color::Black;
        ColourNode::get_mut(&mut self.data, g).color = Color::Red;
        if ColourNode::get(&self.data, p).is_child(&self.data, Side::Right) {
            self.rotate(Side::Left, p);
        } else if ColourNode::get(&self.data, p).is_child(&self.data, Side::Right) {
            self.rotate(Side::Right, p);
        }
    }
    ...
    fn rotate(&mut self, side: Side, n: usize) {
        ...
        if let Some(c) = ColourNode::get(&self.data, n).get_child(side) {
            ColourNode::set_child(&mut self.data, p, c, !side);
        } else {
            match !side {
                Side::Left => ColourNode::get_mut(&mut self.data, p).lchild = None,
                Side::Right => ColourNode::get_mut(&mut self.data, p).rchild = None,
            }
        }
        if let Some(g) = ColourNode::get(&self.data, p).parent {
            ColourNode::get_mut(&mut self.data, n).parent = Some(g);
            let pside = if ColourNode::get(&self.data, p).is_child(&self.data, Side::Left) {
                Side::Left
            } else {
                Side::Right
            };
            ColourNode::set_child(&mut self.data, g, n, pside);
        } else {
            self.root = Some(n);
            ColourNode::get_mut(&mut self.data, n).parent = None
        }
        ColourNode::set_child(&mut self.data, n, p, side);
    }

    pub fn get_size_recursive(&self) -> usize {
        if let Some(root) = self.root {
            ColourNode::get(&self.data, root).get_size(&self.data)
        } else {
            0
        }
    }
    ...
    pub fn get_height(&self) -> usize {
        if let Some(root) = self.root {
            ColourNode::get(&self.data, root).get_height(&self.data)
        } else {
            0
        }
    }
    ...
    fn create_node(&mut self, val: T) -> usize {
        ...
        if self.free.len() > 0 {
            ...
            self.data.push(ColourNode::new(val, loc));
            loc
        }
    }
